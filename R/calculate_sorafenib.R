...
drug_responses_path <- file.path(datapath, 'enlight-data-main', "drug_response_classifications.csv")
drug_responses <- read_csv(drug_responses_path)

drug_name <- "sorafenib"
trial_expression_path <- file.path(datapath, 'enlight-data-main', str_c(drug_name,".csv"))
...
SL_pairs_drug <- bind_rows(SL_pairs_drug1, SL_pairs_drug2) #All the SL pairs of the gene that the drug targets
SL_pairs_drug_list <- SL_pairs_drug[["SL_gene"]]

drug_responses_drug <- drug_responses |>
  filter(Dataset == "Sorafenib") |>
...
avg_gene_expression <- trial_expression |>
  ...
  rowwise() |>
  transmute(SL_gene = index, upper_third = quantile(c_across(where(is.numeric)), 0.66), lower_third = quantile(c_across(where(is.numeric)), 0.33))
...
drug_responses_drug |> 
  group_by(Response) |> 
  ggplot(aes(x = Response, y = score, group = Response, fill = Response)) +
  geom_boxplot() 




wilcoxon <- wilcox.test(as.numeric(drug_responders$score), as.numeric(drug_non_responders$score))